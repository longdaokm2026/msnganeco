...
const teacherAssignment = (item: TeacherAssignment) => ({ ...item, classroom: { id: item.classroom.id, code: item.classroom.code, name: item.classroom.name }, questions: item.questions.map(teacherQuestion), readAloudTask: item.readAloudTask ? { ...item.readAloudTask, maxScore: READ_ALOUD_MAX_SCORE } : null, attemptCount: item._count.attempts, assignedStudentCount: item.classroom._count.enrollments, _count: undefined });
...
export class PrismaAssignmentRepository extends AssignmentRepository {
  ...
  async submit(studentId: string, attemptId: string): Promise<RepositoryResult> {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        await tx.assignmentAttempt.update({ where: { id: attemptId }, data: { status: attempt.assignment.readAloudTask ? AssignmentAttemptStatus.PENDING_MANUAL_GRADE : AssignmentAttemptStatus.AUTO_GRADED, score, maxScore: automaticMaxScore, percentage: objectivePercentage, submittedAt, isLate: late } });
  ...
  private studentAttemptSummary(attempt: { id: string; attemptNumber: number; status: AssignmentAttemptStatus; submittedAt: Date | null; isLate: boolean; answers: { isCorrect: boolean | null }[] }, showScoreImmediately: boolean, totalQuestions: number) {
    const submitted = attempt.status !== AssignmentAttemptStatus.IN_PROGRESS;
    return { id: attempt.id, attemptNumber: attempt.attemptNumber, status: attempt.status, submittedAt: attempt.submittedAt, isLate: attempt.isLate, ...(showScoreImmediately && submitted ? { objectiveResult: objectiveResult(attempt.answers, totalQuestions) } : {}) };
  }
  private async attemptView(client: Prisma.TransactionClient | typeof prisma, attemptId: string, studentId: string, resultOnly = false) {
    ...
    return { id: attempt.id, assignmentId: attempt.assignmentId, attemptNumber: attempt.attemptNumber, status: attempt.status, startedAt: attempt.startedAt, submittedAt: attempt.submittedAt, isLate: attempt.isLate, pendingManualGrade: pendingManual, assignment: studentAssignment(attempt.assignment, reveal), answers: attempt.answers.map((answer) => { const question = questions.get(answer.questionId); return { questionId: answer.questionId, answer: answer.answer, updatedAt: answer.updatedAt, ...(showGrade && question ? { result: humanReadableQuestionResult(question, answer, reveal) } : {}) }; }), readAloudSubmission: attempt.readAloudSubmission ? { id: attempt.readAloudSubmission.id, durationSeconds: attempt.readAloudSubmission.durationSeconds, submittedAt: attempt.readAloudSubmission.submittedAt, audioAttachment: attempt.readAloudSubmission.audioAttachment, audioUrl: `/assignment-read-aloud-submissions/${attempt.readAloudSubmission.id}/audio`, ...(showGrade ? { score: decimal(attempt.readAloudSubmission.score), feedback: attempt.readAloudSubmission.feedback, gradedAt: attempt.readAloudSubmission.gradedAt } : {}) } : null, ...(showGrade ? { objectiveResult: objectiveResult(attempt.answers, attempt.assignment.questions.length) } : {}), resultOnly };
  }
  ...
  async results(teacherId: string, assignmentId: string): Promise<RepositoryResult> {
    ...
    const attempts = await prisma.assignmentAttempt.findMany({ where: { assignmentId, status: { in: submittedStatuses } }, include: { answers: { select: { isCorrect: true } }, readAloudSubmission: { select: { score: true, gradedAt: true } } }, orderBy: [{ studentId: "asc" }, { attemptNumber: "desc" }] });
    const latest = new Map<string, typeof attempts[number]>();
    for (const attempt of attempts) if (!latest.has(attempt.studentId)) latest.set(attempt.studentId, attempt);
    const enrolled = assignment.classroom.enrollments.map((item) => item.student.user); const submitted = [...latest.values()]; const percentages = submitted.map((item) => objectiveResult(item.answers, assignment._count.questions).percentage);
    return { status: "OK", value: { summary: { enrolled: enrolled.length, submitted: latest.size, notSubmitted: enrolled.length - latest.size, averagePercentage: percentages.length ? percentages.reduce((sum, value) => sum + value, 0) / percentages.length : null, highestPercentage: percentages.length ? Math.max(...percentages) : null, lowestPercentage: percentages.length ? Math.min(...percentages) : null, lateSubmissions: submitted.filter((item) => item.isLate).length }, students: enrolled.map((student) => { const attempt = latest.get(student.id); return { student, status: attempt ? "SUBMITTED" : "NOT_SUBMITTED", ...(attempt ? { ...this.studentAttemptSummary(attempt, true, assignment._count.questions), audioResult: attempt.readAloudSubmission ? { status: attempt.readAloudSubmission.gradedAt ? "GRADED" : "PENDING_GRADE", score: decimal(attempt.readAloudSubmission.score), maxScore: READ_ALOUD_MAX_SCORE } : null } : {}) }; }) } };
  }
