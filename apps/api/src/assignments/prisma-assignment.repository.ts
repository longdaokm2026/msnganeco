...
const questionSelect = { id: true, passageId: true, listeningTrackId: true, type: true, section: true, position: true, prompt: true, explanation: true, points: true, required: true, config: true, createdAt: true, updatedAt: true } satisfies Prisma.AssignmentQuestionSelect;
...
const listeningTrackSelect = { id: true, assignmentId: true, title: true, instructions: true, transcript: true, transcriptVisibility: true, maxPlayCount: true, allowSeeking: true, position: true, audioAttachment: { select: { id: true, fileName: true, fileType: true, fileSize: true } }, _count: { select: { questions: true } } } satisfies Prisma.AssignmentListeningTrackSelect;
const assignmentSelect = {
  ...
  listeningTracks: { select: listeningTrackSelect, orderBy: { position: "asc" as const } },
...
const teacherAssignment = (item: TeacherAssignment) => ({ ...item, classroom: { id: item.classroom.id, code: item.classroom.code, name: item.classroom.name }, questions: item.questions.map(teacherQuestion), listeningTracks: item.listeningTracks.map((track) => ({ ...track, audioAttachment: track.audioAttachment ? { ...track.audioAttachment, audioUrl: `/assignments/${item.id}/listening-tracks/${track.id}/audio` } : null, questionCount: track._count.questions, _count: undefined })), readAloudTask: item.readAloudTask ? { ...item.readAloudTask, maxScore: READ_ALOUD_MAX_SCORE } : null, writingTask: item.writingTask ? { ...item.writingTask, maxScore: 10 } : null, attemptCount: item._count.attempts, assignedStudentCount: item.classroom._count.enrollments, _count: undefined });
const studentQuestion = (item: TeacherAssignment["questions"][number], reveal = false) => ({ id: item.id, passageId: item.passageId, listeningTrackId: item.listeningTrackId, type: item.type, section: item.section, position: item.position, prompt: item.prompt, points: decimal(item.points), required: item.required, config: studentConfig(item.type, item.config), ...(reveal ? { explanation: item.explanation } : {}) });
const studentAssignment = (item: TeacherAssignment, reveal = false, submitted = false) => ({ id: item.id, classroomId: item.classroomId, lessonId: item.lessonId, title: item.title, description: item.description, type: item.type, status: item.status, dueAt: item.dueAt, allowLateSubmission: item.allowLateSubmission, maxAttempts: item.maxAttempts, timeLimitMinutes: item.timeLimitMinutes, showScoreImmediately: item.showScoreImmediately, showAnswersAfterSubmit: item.showAnswersAfterSubmit, showLeaderboard: item.showLeaderboard, generationMode: item.generationMode, publishedAt: item.publishedAt, classroom: { id: item.classroom.id, code: item.classroom.code, name: item.classroom.name }, lesson: item.lesson, listeningTracks: item.listeningTracks.map((track) => ({ id: track.id, title: track.title, instructions: track.instructions, transcriptVisibility: track.transcriptVisibility, maxPlayCount: track.maxPlayCount, allowSeeking: track.allowSeeking, position: track.position, audioReady: Boolean(track.audioAttachment), questionCount: track._count.questions, ...(submitted && track.transcriptVisibility === "AFTER_SUBMIT" ? { transcript: track.transcript } : {}) })), readAloudTask: item.readAloudTask ? { ...item.readAloudTask, maxScore: READ_ALOUD_MAX_SCORE } : null, writingTask: item.writingTask ? { ...item.writingTask, maxScore: 10, translationItems: item.writingTask.translationItems.map((translationItem) => ({ id: translationItem.id, position: translationItem.position, sourceText: translationItem.sourceText })) } : null, passages: item.passages, questions: item.questions.map((question) => studentQuestion(question, reveal)) });
...
export class PrismaAssignmentRepository extends AssignmentRepository {
  ...
  async transition(teacherId: string, assignmentId: string, action: "publish" | "close" | "archive"): Promise<RepositoryResult> {
    return prisma.$transaction(async (tx) => {
      ...
      const publishError = action === "publish" ? assignmentPublishError({ title: item.title, questionCount: item.questions.length, listeningTracks: item.listeningTracks.map((track) => ({ hasAudio: Boolean(track.audioAttachment), questionCount: track._count.questions })), readAloudTask: item.readAloudTask ? { readingText: item.readAloudTask.readingText, maxScore: Number(item.readAloudTask.maxScore) } : null, writingTask: item.writingTask ? { type: item.writingTask.type, prompt: item.writingTask.prompt, translationItemCount: item.writingTask.translationItems.length } : null }) : null;
  ...
  async addQuestion(teacherId: string, assignmentId: string, input: QuestionInput): Promise<RepositoryResult> { return prisma.$transaction(async (tx) => { if (!await this.draft(tx, teacherId, assignmentId)) return { status: "NOT_FOUND" }; if (input.passageId && input.listeningTrackId) return { status: "INVALID", message: "Câu hỏi không thể đồng thời thuộc Reading và Listening." }; if (input.passageId && !await tx.assignmentPassage.findFirst({ where: { id: input.passageId, assignmentId }, select: { id: true } })) return { status: "INVALID", message: "Đoạn đọc không thuộc bài tập." }; if (input.listeningTrackId && !await tx.assignmentListeningTrack.findFirst({ where: { id: input.listeningTrackId, assignmentId }, select: { id: true } })) return { status: "INVALID", message: "Đoạn Listening không thuộc bài tập." }; if (input.section === "LISTENING" && !input.listeningTrackId) return { status: "INVALID", message: "Câu Listening phải thuộc một đoạn nghe." }; const aggregate = await tx.assignmentQuestion.aggregate({ where: { assignmentId }, _max: { position: true } }); const item = await tx.assignmentQuestion.create({ data: { assignmentId, passageId: input.passageId ?? null, listeningTrackId: input.listeningTrackId ?? null, type: input.type, section: input.section, prompt: input.prompt, explanation: input.explanation ?? null, points: input.points, required: input.required, config: input.config as Prisma.InputJsonValue, position: (aggregate._max.position ?? -1) + 1 }, select: questionSelect }); return { status: "OK", value: teacherQuestion(item) }; }); }
  async updateQuestion(teacherId: string, assignmentId: string, questionId: string, input: QuestionInput): Promise<RepositoryResult> { return prisma.$transaction(async (tx) => { if (!await this.draft(tx, teacherId, assignmentId)) return { status: "NOT_FOUND" }; const existing = await tx.assignmentQuestion.findFirst({ where: { id: questionId, assignmentId }, select: { id: true } }); if (!existing) return { status: "NOT_FOUND" }; if (input.passageId && input.listeningTrackId) return { status: "INVALID", message: "Câu hỏi không thể đồng thời thuộc Reading và Listening." }; if (input.passageId && !await tx.assignmentPassage.findFirst({ where: { id: input.passageId, assignmentId }, select: { id: true } })) return { status: "INVALID", message: "Đoạn đọc không thuộc bài tập." }; if (input.listeningTrackId && !await tx.assignmentListeningTrack.findFirst({ where: { id: input.listeningTrackId, assignmentId }, select: { id: true } })) return { status: "INVALID", message: "Đoạn Listening không thuộc bài tập." }; if (input.section === "LISTENING" && !input.listeningTrackId) return { status: "INVALID", message: "Câu Listening phải thuộc một đoạn nghe." }; const item = await tx.assignmentQuestion.update({ where: { id: questionId }, data: { passageId: input.passageId ?? null, listeningTrackId: input.listeningTrackId ?? null, type: input.type, section: input.section, prompt: input.prompt, explanation: input.explanation ?? null, points: input.points, required: input.required, config: input.config as Prisma.InputJsonValue }, select: questionSelect }); return { status: "OK", value: teacherQuestion(item) }; }); }
  ...
  private async attemptView(client: Prisma.TransactionClient | typeof prisma, attemptId: string, studentId: string, resultOnly = false) {
    ...
    const playbacks = await client.assignmentListeningPlayback.findMany({ where: { attemptId }, select: { trackId: true } });
    const playCounts = new Map<string, number>(); for (const playback of playbacks) playCounts.set(playback.trackId, (playCounts.get(playback.trackId) ?? 0) + 1);
    const assignment = studentAssignment(attempt.assignment, reveal, submitted);
    return { id: attempt.id, assignmentId: attempt.assignmentId, attemptNumber: attempt.attemptNumber, status: attempt.status, startedAt: attempt.startedAt, submittedAt: attempt.submittedAt, durationMs: attempt.submittedAt ? Math.max(0, attempt.submittedAt.getTime() - attempt.startedAt.getTime()) : null, expiresAt: attempt.assignment.timeLimitMinutes ? new Date(attempt.startedAt.getTime() + attempt.assignment.timeLimitMinutes * 60_000) : null, isLate: attempt.isLate, pendingManualGrade: pendingManual, assignment: { ...assignment, listeningTracks: assignment.listeningTracks.map((track) => ({ ...track, playCount: playCounts.get(track.id) ?? 0 })) }, answers: attempt.answers.map((answer) => { const question = questions.get(answer.questionId); return { questionId: answer.questionId, answer: answer.answer, updatedAt: answer.updatedAt, ...(showGrade && question ? { result: humanReadableQuestionResult(question, answer, reveal) } : {}) }; }), readAloudSubmission: attempt.readAloudSubmission ? { id: attempt.readAloudSubmission.id, durationSeconds: attempt.readAloudSubmission.durationSeconds, submittedAt: attempt.readAloudSubmission.submittedAt, audioAttachment: attempt.readAloudSubmission.audioAttachment, audioUrl: `/assignment-read-aloud-submissions/${attempt.readAloudSubmission.id}/audio`, ...(showGrade ? { score: decimal(attempt.readAloudSubmission.score), feedback: attempt.readAloudSubmission.feedback, gradedAt: attempt.readAloudSubmission.gradedAt } : {}) } : null, writingSubmission: writingSubmissionView(attempt.assignment.writingTask, attempt.writingSubmission, showGrade), ...(showGrade && attempt.assignment.questions.length > 0 ? { objectiveResult: objectiveResult(attempt.answers, attempt.assignment.questions.length) } : {}), resultOnly };
  }
