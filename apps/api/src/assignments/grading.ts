...
const mcq = new Set<AssignmentQuestionType>([AssignmentQuestionType.VOCAB_MULTIPLE_CHOICE, AssignmentQuestionType.GRAMMAR_MULTIPLE_CHOICE, AssignmentQuestionType.READING_MULTIPLE_CHOICE]);
const textAnswer = new Set<AssignmentQuestionType>([AssignmentQuestionType.VOCAB_FILL_BLANK, AssignmentQuestionType.GRAMMAR_FILL_BLANK, AssignmentQuestionType.GRAMMAR_ERROR_CORRECTION, AssignmentQuestionType.READING_SHORT_ANSWER]);
const sectionFor = (type: AssignmentQuestionType) => type.startsWith("VOCAB_") ? AssignmentSection.VOCABULARY : type.startsWith("GRAMMAR_") ? AssignmentSection.GRAMMAR : AssignmentSection.READING;
...
export function validateQuestion(input: QuestionInput) {
  ...
  } else if (input.type === AssignmentQuestionType.VOCAB_MATCHING) {
...
export function gradeQuestion(question: GradeQuestion, rawAnswer: unknown): GradeResult {
  ...
  if (question.type === AssignmentQuestionType.READING_TRUE_FALSE_NOT_GIVEN) {
  ...
  if (question.type === AssignmentQuestionType.VOCAB_MATCHING) {
