...
export function assignmentPublishError(input: { title: string; questionCount: number; readAloudTask?: { readingText: string; maxScore: number } | null; writingTask?: { type: "ESSAY" | "TRANSLATION_VI_EN" | "TRANSLATION_EN_VI"; prompt: string | null; translationItemCount: number } | null }) {
  if (!input.title.trim() || (!input.questionCount && !input.readAloudTask && !input.writingTask)) return "Cần có tiêu đề và ít nhất một câu hỏi, Speaking hoặc Writing trước khi xuất bản.";
