...
type LessonDocumentKey = "title" | "summary" | "mainContent" | "theory" | "vocabulary" | "grammar" | "examples" | "reviewNotes" | "homeworkNotes";
type ImportPreview = { fields: Partial<Record<LessonDocumentKey, string>>; foundSections: string[]; missingSections: string[]; warnings: string[]; vocabularyCount: number };
...
export default function TeacherLessonManager({ apiUrl, accessToken, onBack, onCreateAssignment }: { apiUrl: string; accessToken: string; onBack: () => void; onCreateAssignment?: (prefill: { classroomId: string; lessonId: string; title: string }) => void }) {
  ...
  async function open(sessionId: string) {
    if (busy) return;
    try { setPageError(""); setFeedback(null); setSavedAt(null); setImportPreview(null); acceptLesson(await api(`/sessions/${sessionId}/lesson`)); }
  ...
  async function downloadWord(path: string, fileName: string, nextOperation: "template" | "export") {
    if (busy) return;
    setOperation(nextOperation); setFeedback(null);
    try {
      const response = await fetch(`${apiUrl}${path}`, { headers });
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        throw new Error(Array.isArray(body?.message) ? body.message.join(" ") : body?.message ?? "Không thể tải file Word.");
      }
      const url = URL.createObjectURL(await response.blob());
      const anchor = document.createElement("a"); anchor.href = url; anchor.download = fileName; anchor.click(); URL.revokeObjectURL(url);
      setFeedback({ kind: "success", text: nextOperation === "template" ? "✓ Đã tải mẫu Word." : "✓ Đã xuất bài học ra Word." });
    } catch (reason) { setFeedback({ kind: "error", text: friendlyError(reason, "Không thể tải file Word.") }); }
    finally { setOperation(null); }
  }
  ...
  async function importWord(event: ChangeEvent<HTMLInputElement>) {
    const input = event.target; const file = input.files?.[0];
    if (!lesson || !file || busy) return;
    if (!/\.docx$/i.test(file.name)) { setFeedback({ kind: "error", text: "Chỉ hỗ trợ file Word định dạng .docx." }); input.value = ""; return; }
    if (file.size > 5 * 1024 * 1024) { setFeedback({ kind: "error", text: "File Word vượt quá giới hạn 5 MB." }); input.value = ""; return; }
    const form = new FormData(); form.append("file", file); setOperation("import"); setFeedback(null); setImportPreview(null);
    try {
      const preview = await api<ImportPreview>(`/sessions/${lesson.sessionId}/lesson/import-docx/preview`, { method: "POST", body: form });
      setImportPreview(preview);
      setFeedback({ kind: "success", text: "✓ Đã đọc file Word. Hãy kiểm tra bản xem trước trước khi áp dụng." });
    } catch (reason) { setFeedback({ kind: "error", text: friendlyError(reason, "Không thể đọc file Word. Vui lòng dùng đúng file mẫu.") }); }
    finally { setOperation(null); input.value = ""; }
  }

  function applyImport() {
    if (!lesson || !importPreview) return;
    setLesson({ ...lesson, ...importPreview.fields });
    setImportPreview(null);
    setFeedback({ kind: "success", text: "✓ Đã đưa nội dung Word vào biểu mẫu. Hãy kiểm tra và bấm Lưu." });
  }
    ...
    <div className="lesson-workspace">
      ...
      <section className="lesson-editor">{lesson ? <form onSubmit={save}>
        ...
        <section className="lesson-word-tools">
          <div><h3>Tài liệu Word</h3><p>Soạn bằng mẫu cố định, chỉ nhập nội dung chữ. Import tạo bản xem trước và không tự lưu bài học.</p></div>
          <div className="lesson-word-actions">
            <button type="button" disabled={busy} onClick={() => void downloadWord("/teacher/lessons/docx-template", "Mau-bai-hoc-Ms-Ngan-English.docx", "template")}>{operation === "template" ? "Đang tải..." : "Tải mẫu Word"}</button>
            <label className={`lesson-word-import${busy ? " disabled" : ""}`}>{operation === "import" ? "Đang đọc..." : "Import Word"}<input type="file" hidden disabled={busy} accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => void importWord(event)} /></label>
            <button type="button" disabled={busy || isDirty} title={isDirty ? "Hãy lưu thay đổi trước khi xuất Word." : "Xuất nội dung bài học đã lưu"} onClick={() => void downloadWord(`/sessions/${lesson.sessionId}/lesson/export-docx`, `${lesson.title || "bai-hoc"}.docx`, "export")}>{operation === "export" ? "Đang xuất..." : "Export Word"}</button>
          </div>
        </section>
        {importPreview && <section className="lesson-import-preview" aria-live="polite">
          <div className="lesson-import-preview-heading"><div><h3>Xem trước nội dung Word</h3><p>Đã nhận {importPreview.foundSections.length} đầu mục{importPreview.vocabularyCount ? ` · ${importPreview.vocabularyCount} từ vựng` : ""}.</p></div><span>Chưa lưu</span></div>
          {importPreview.warnings.map((warning) => <p className="lesson-import-warning" key={warning}>{warning}</p>)}
